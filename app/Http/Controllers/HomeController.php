...
class HomeController extends Controller
{
    ...
    public function findAsteroid(Request $request){
        ...
        if($data->error){
            return redirect()->back()->with('error', $data->error->message);
        }
        ...
        if(isset($data->near_earth_objects)){
            foreach($data->near_earth_objects as $newdata){
                ...
                $closest_astroid = [];
                $max_speed = 0;
                $closest_astroid_distance = $newdata[0]->close_approach_data[0]->miss_distance->kilometers;
                $average_size = 0;
                ...
                array_push($fastest_astroid_array, $max_speed);
                array_push($closest_astroid_array, $closest_astroid_distance);
